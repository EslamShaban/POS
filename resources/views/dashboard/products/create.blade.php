            ...
            <form action="{{route('dashboard.products.store')}}" method="POST" enctype="multipart/form-data">
                  ...
                  @foreach (config('translatable.locales') as $locale)
                      <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="name" name="{{$locale}}[name]" value="{{old($locale . '.name')}}">
                      </div> 
                      <div class="form-group">
                        <label for="description">@lang('site.' . $locale . '.description')</label>
                        <textarea class="form-control ckeditor" id="description" name="{{$locale}}[description]">{{old($locale . '.description')}}</textarea>
                      </div> 
                  @endforeach
                  <div class="form-group">
                ...
                <div class="form-group">
                    <img src="{{asset('uploads/products_image/default.png')}}" class="imgPreview img-thumbnail" style="width:100px" alt="" >
                </div>
                <div class="form-group">
                  <label for="purchasePrice">@lang('site.purchase_price')</label>
                  <input type="number" class="form-control" step="0.01" id="purchasePrice" name="purchase_price" value="{{old('purchase_price')}}">
                </div> 
                <div class="form-group">
                  <label for="salePrice">@lang('site.sale_price')</label>
                  <input type="number" class="form-control" step="0.01" id="salePrice" name="sale_price" value="{{old('sale_price')}}">
                </div> 
                <div class="form-group">
